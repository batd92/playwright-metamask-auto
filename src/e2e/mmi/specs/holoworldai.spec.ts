...
import { HoloworldaiPage } from '../service/mmi-holoworldai-page';
...
const signInAndConnectDapp = async (page: any, context: any, data: any) => {
    ...
    await networkPage.addNetwork(network);

    // connect HoloWorldAI
    const holoworldaiPage = new HoloworldaiPage(page);
    await holoworldaiPage.gotoReferralCode();
    await holoworldaiPage.connectAndCreateUser(context);
    await sleep(2000);
    
    //await holoworldaiPage.operateOnTheWeb(data);
    await sleep(1000);
};
